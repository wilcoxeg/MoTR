...
file_prefix = paste0("/Users/cui/Desktop/motr_lc_new_", as.character(rate),"/")
...
raw_df = df %>%
  ...
  dplyr::select(expr_id, cond_id, para_nr, word, word_nr, subj, sent_rg_id, sent_rg, first_duration, total_duration, gaze_duration, go_past_time, FPReg)
...
motr_df = raw_df %>%
  ...
  mutate(fixed = if_else(total_duration == 0, 0, 1)) %>%
    group_by(para_nr, subj, cond_id, expr_id) %>%
    mutate(n_fixed = sum(fixed),
           n_words = n()) %>%
  ungroup() %>%
  mutate(fix_threshold = n_fixed > (n_words / 5)) %>%
  filter(fix_threshold == T) %>%
  
  gather(metric, value, 9:13) %>%
  ...
  group_by(metric, condition, word) %>%
  # group_by(metric) %>%
  ...
  dplyr::select(-fixed, -n_fixed, -n_words, -fix_threshold, -outlier, -outlier_discard)
View(motr_df)
...
motr_clean_df = motr_df %>%
  ...
  ungroup() %>%
  mutate(measure = "MoTR") %>%
  mutate(value = ifelse(metric == "FPReg" & value > 0, 1, value))   # If there is a regression initiated from this region, the FPReg is 1.
# View(motr_clean_df)
    ...
    for(mes in c("MoTR", "G-Maze", "A-Maze", "SPR")) {
      ...
      for(i in -1:2) {
        ...
        if(nrow(temp_df) > 0) {
          ...
          print(paste("Stats for:", i, exp, met, mes, sep = " "))
...
write.csv(stats_df, "/Users/cui/Desktop/stats_df.csv")
...
plotting_df %>%
  ...
  scale_x_continuous(breaks = c(-1, 0, 1, 2), labels= c("Pre-Critical", "Critical +0", "Critical +1", "Critcal +2")) +
  labs(color = "", linetype = "") +
...
# relationship between FPReg and first-pass reading times
```{r}

motr_regression_df = motr_df %>% 
  filter(metric == "gaze_duration" | metric == "FPReg") %>%
  spread(metric, value) %>%
  filter(gaze_duration != 0) %>%
  mutate(gd_normalized = gaze_duration/ nchar(word))


View(motr_regression_df)
```


```{r}

motr_sum <- motr_regression_df %>%
  group_by(FPReg) %>%
  summarise(mean_gd_normalized = mean(gd_normalized),
            se_gd_normalized = std.error(gd_normalized) )
  
ggplot(motr_sum, aes(x = factor(FPReg), y = mean_gd_normalized)) +
  geom_bar(stat = "identity", fill = "grey", width = 0.3) +
  geom_errorbar(aes(ymin = mean_gd_normalized - 1.96 * se_gd_normalized, 
                    ymax = mean_gd_normalized + 1.96 * se_gd_normalized),
                width = 0.2, color = "black") +
  labs(x = "FPReg", y = "Mean Gaze Duration Normalized by length") +
  theme_minimal() +
  ...
  ggtitle("Mean Normalized Gaze Duration by FPReg")

```


```{r}
motr_relation_lm = lmer(gd_normalized ~ FPReg + (FPReg | subj) + (FPReg | item_id), data = motr_regression_df)

summary(motr_relation_lm)
```
