...
motr_df = raw_df %>%
  ...
  ungroup() %>%
  mutate(outlier_discard = if_else(metric %in% c("first_duration", "gaze_duration", "total_fixation", "go_past_time") & outlier == T, F, T)) %>%
...
motr_clean_df = motr_df %>%
  ...
  ungroup() %>%
  mutate(measure = "MoTR") %>%
  mutate(value = ifelse(metric == "FPReg" & value > 0, 1, value))   # If there is a regression initiated from this region, the FPReg is 1, instead of a prob, right?
