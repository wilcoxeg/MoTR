...
class FeatureExtractor:
    ...
    def get_right_bounded_rt_manual(self):
        ...
        rb_rt_df = pd.DataFrame(columns=['cond_id', 'para_nr', 'word_nr', 'right_bounded_rt2'])
        for name, group in input_df_f_grouped:
            ...
            for i in range(len(indices)):
                if i == 0:
                    for j in range(indices.iloc[i]-1, -1, -1):
                        if j not in previous_indices:
                            flag = False
                            break
                if not flag:
                    break
                if i == len(indices) - 1:
                    if duration.iloc[i] > self.threshold:
                        sum_duration += duration.iloc[i]
                else:
                    if duration.iloc[i] > self.threshold:
                        sum_duration += duration.iloc[i]
                    for j in range(indices.iloc[i]+1, indices.iloc[i+1]):
                        if j not in previous_indices:
                            flag = False
                            break
                if not flag:
        ...
        self.output_df = self.output_df.merge(rb_rt_df, on=['cond_id', 'para_nr', 'word_nr'], how='left')
...
# input_paths_l = Path('../fixations_merged/reader_9_merged_denoise.csv')
# output_path_l = Path('../junk')
# obj = FeatureExtractor(input_paths_trial, input_paths_l, output_path_l, 150)

# obj.check_comprehension_answer()
# obj.get_first_duration()
# obj.get_total_duration()
# obj.get_gaze_duration()
# obj.get_right_bounded_rt()
# # obj.get_right_bounded_rt_manual()
# obj.get_go_pass_time()
# obj.get_binary()
# obj.write_out()
