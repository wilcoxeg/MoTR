...
file_prefix = "../reading_measures/motr_lc_200"
...
for(i in -2:3) {
  for(m in c("first_duration", "gaze_duration", "go_past_time", "total_duration", "FPReg")){
    temp_df = motr_coord_df %>% filter(metric == m) %>% filter(word_nr == i) %>%
      mutate(item_id = as.factor(item_id), subj = as.factor(subj), condition = if_else(condition == "comma", 1, -1))
    if (m == "FPReg") {
...
motr_coord_plotting_df %>%
  ...
  ggplot(aes(x = word_nr, y = m, color = condition)) +
    geom_rect(aes(xmin = -0.5, xmax = 2.5, ymin = if_else(metric=="First Pass Regression (prob)", 0, 100), ymax = if_else(metric=="First Pass Regression (prob)", 0.08, 780)), fill=alpha("white", 0), color = "#45ef70", linetype = "dotted") +
    geom_point() +
    geom_errorbar(aes(ymax = upper, ymin = lower), width = 0) +
    geom_line() +
    geom_text(aes(label = sig, y = if_else(metric=="First Pass Regression (prob)", 0.06, 750)), color = "black") +
    geom_text(aes(label = " ", y = if_else(metric=="First Pass Regression (prob)", 0.06, 770))) +
    #geom_text(aes(label = word, y = if_else(cond_id == "Comma", 3000, 3500)), size = 2) +
    #facet_grid(para_nr~cond_id) +
  ylab("Reading Time") +
  xlab("Condition") +
  scale_x_continuous(breaks=-2:3, labels=c("his", "wife", "ran", "away", "from", "the")) +
...
motr_rc_plotting_df %>%
  ...
  mutate(sig = case_when( pval >= 0.1 ~ " ",
                      ...
                      pval < 0.001  ~ "***")) %>%
  ggplot(aes(x = word_nr, y = m, color = condition)) +
    geom_rect(aes(xmin = -0.5, xmax = 2.5, ymin = if_else(metric=="First Pass Regression (prob)", 0, 300), ymax = if_else(metric=="First Pass Regression (prob)", 0.09, 850)), fill=alpha("white", 0), color = "#45ef70", linetype = "dotted") +
    geom_point() +
    geom_errorbar(aes(ymax = upper, ymin = lower), width = 0.3) +
    geom_line() +
    geom_text(aes(label = sig, y = if_else(metric=="First Pass Regression (prob)", 0.08, 750)), color = "black") +
    geom_text(aes(label = " ", y = if_else(metric=="First Pass Regression (prob)", 0.085, 770))) +
    #geom_text(aes(label = word, y = if_else(cond_id == "Comma", 3000, 3500)), size = 2) +
    #facet_grid(para_nr~cond_id) +
  ylab("Reading Time") +
  xlab("Condition") +
  scale_x_continuous(breaks=-2:3, labels=c( "who", "got", "himself\n herself", "a", "boat", "was")) +
...
x = as.character(summary(model))
y = str_extract_all(x, "[0-9]+.[0-9]+")[-1]

y = as.double(substr(x, nchar(x)-61, nchar(x)-55))
...
table(temp_df$condition, temp_df$value)
...
motr_adv_plotting_df %>%
  ...
  ylab("Reading Time") +
  xlab("Condition") +
  scale_x_continuous(breaks=-2:3, labels=c("she", "took", "last/next", "week,", "but", "she")) +
  facet_wrap(.~metric, scales="free_y") +
  theme(
    legend.position = "none",
    axis.text.x = element_text(angle = 45, hjust = 1)
