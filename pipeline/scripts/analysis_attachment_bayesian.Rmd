...
# <<<<<<< HEAD:pipeline/scripts/analysis_attachment.Rmd
...
# >>>>>>> 6d551a95f197cb2ecb8d26837d651e58c06a7b16:pipeline/scripts/analysis_attachment_fullcomp.Rmd
...
# set priors
priors_rt_motr <- c(
  prior(normal(6.5, 0.1), class = Intercept),
        ...
        if(nrow(temp_df) > 0) {
          ...
          if (met == "FPReg") {
            ...
            model = brms::brm(value ~ condition + (condition | subj) + (condition | item_id), 
                              ...
                              chains = 4,
                              file = paste0("./bayesian_models_lc/model_all_data/", exp, "_", met, "_", mes, "_", i), 
                              cores = 4, 
                              backend = "cmdstanr", 
                              prior = priors_FPReg)
...
write.csv(stats_df, "./stats_bayesian.csv")
...
stats_df_bayesian <- read.csv("stats_bayesian.csv")

plotting_df = merge(comb_agg_df, stats_df_bayesian, all=T, by = c("sent_rg_id", "metric", "experiment", "measure")) %>%
  mutate(mean = ifelse(is.na(mean), m, mean),
        lower_95_ms = ifelse(is.na(lower_95_ms), lower, lower_95_ms),
        upper_95_ms = ifelse(is.na(upper_95_ms), upper, upper_95_ms)) %>%
  dplyr::select(-m, -sd, -upper, -lower, -X) %>%
  mutate(m = mean,
         lower = lower_95_ms,
         upper = upper_95_ms) %>%

  # mutate(sig = if_else(measure != "Eye Tracking" & (upper < 0 | lower > 0), "*", "")) %>%
  mutate(sig = if_else(measure != "Eye Tracking" & (upper < 0 | lower > 0), paste(round(m, 2), "\n[", round(lower, 2), ", ", round(upper, 2), "]", sep = ""), "")) %>%
...
plotting_df %>%
  ...
  ggplot(aes(x = sent_rg_id, y = m, color = measure, linetype = measure)) +
    ...
    geom_line() +
    geom_text(aes(label = sig, y = if_else(metric == "Regression Prob", 0.12, 250)), color = "black", size = 1.2) +
...
ggsave(paste0("../visualization/paper/target_results_bayesisan_all_data", rate, ".pdf"), device="pdf", width = 8, height = 5.5)
...
ggsave(paste0("../visualization/paper/target_comp_bayesian_all_data", rate, ".pdf"), device="pdf", width = 8, height = 4.5)
