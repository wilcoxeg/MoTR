...
file_prefix = "/Users/ethan/Desktop/motr_provo_160/"
...
provo_eyetr_grouped_df = merge(provo_eyetracking_subj2_df, provo_eyetracking_subj1_df, by=c("text_id", "word_text_idx", "metric")) %>%
  ...
  group_by(metric) %>%
    mutate(motr_outlier = if_else(value_1 > (mean(value_1) + 3 * sd(value_1) ), T, F)) %>%
    filter(motr_outlier == F) %>%
    mutate(eyetr_outlier = if_else(value_2 > (mean(value_2) + 3 * sd(value_2) ), T, F)) %>%
...
provo_df = merge(provo_df, motr_agg_df, by=c("text_id", "word_text_idx", "metric")) %>%
  ...
  group_by(metric) %>%
    ...
    filter(motr_outlier == F) %>%
    mutate(eyetr_outlier = if_else(eyetr_value > (mean(eyetr_value) + 3 * sd(eyetr_value) ), T, F)) %>%
    filter(eyetr_outlier == F) %>%
  ungroup() %>%
...
gd_df = provo_df %>% filter(metric == "FPReg") %>% spread(measure, value)
print(cor.test(gd_df$eyetr_value, gd_df$motr_value)$estimate)

cor_df = provo_eyetr_grouped_df %>% filter(metric == "FPReg") %>% spread(measure, value)
print(cor.test(cor_df$value_1, cor_df$value_2)$estimate)
