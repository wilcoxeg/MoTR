...
provo_eyetracking_df = provo_raw_df %>%
  dplyr::select(Participant_ID, Text_ID, Word_Number, Sentence_Number, Word_In_Sentence_Number, Word, IA_FIRST_FIXATION_DURATION,IA_FIRST_RUN_DWELL_TIME, IA_DWELL_TIME, IA_REGRESSION_PATH_DURATION, IA_REGRESSION_OUT) %>%
...
motr_attach_df = motr_df %>%
  ...
  filter(metric != "first_duration", metric != "FPReg") %>%
