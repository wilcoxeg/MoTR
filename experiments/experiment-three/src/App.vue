<!-- Window is small and fixed, 80px, pointer cursor bellow, more gradual blurry effect on surrounding words. -->
<!-- Comprehension questions appear afterwards in different slides -->
    ...
    <Experiment title="Test blur text with mouse tracking ">
      ...
      <Screen :title="'Welcome'" class="instructions" :validations="{
          ...
          <br>
            ...
            <br />
            <button style="transform: none;" @click="$magpie.addExpData({ SubjectId: $magpie.measurements.SubjectID}); $magpie.nextScreen()">
              Proceed
            </button>
          ...
          <Slide>
            ...
            <div class="readingText" @mousemove="moveCursor" @mouseleave="changeBack">
              <template v-for="(word, index) of trial.text.split(' ')">
                <span :class="{ bold: currentIndex === index }" :key="index" @mouseover="recordWord(index)" :data-index="index">
                  {{ word }}
                </span>
              </template>
            </div>
            ...
            <button @click="$magpie.nextSlide()">
              Done
            </button>
          </Slide>
         
          <Slide>
            <div style = "position:absolute; top: 30%; text-align: center; min-width: -webkit-fill-available;">
              <template>
                <form>
  ...
  import localCoherence_items from '../trials/localCoherence_items.tsv';
  ...
  export default {
    ...
    methods: {
      ...
      moveCursor(e) {
        ...
        this.$el.querySelector(".oval-cursor").style.top = `${y-8}px`;
