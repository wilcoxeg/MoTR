...
plotting_df = merge(comb_agg_df, stats_df_bayesian, all=T, by = c("sent_rg_id", "metric", "experiment", "measure")) %>%
  mutate(lower_95_ms = if_else(metric != "FPReg", round(lower_95_ms, digits=0), round(lower_95_ms, digits=2))) %>%
  mutate(upper_95_ms = if_else(metric != "FPReg", round(upper_95_ms, digits=0), round(upper_95_ms, digits=2))) %>%
  
  mutate(sig = if_else(measure != "Eye Tracking" & (upper_95_ms < 0 | lower_95_ms > 0), T, F)) %>%
  mutate(cris = if_else(measure != "Eye Tracking" & sig == T, paste0("[",lower_95_ms,",",upper_95_ms,"]"), "")) %>%
...
plotting_df %>%
  ...
  ggplot(aes(x = sent_rg_id, y = m, color = measure, linetype = measure)) +
    ...
    geom_line() +
    #geom_text(aes(label = cris, y = if_else(metric == "Regression Prob", 0.13, 190) ), size = 2) +
    geom_text(aes(label = cris, y = if_else(metric == "Regression Prob", m + 0.05, m + 80) ), size = 2) +
...
ggsave(paste0("../visualizations/target_results_cris", rate, ".pdf"), device="pdf", width = 8, height = 5.5)
...
plotting_df %>%
  ...
  ggplot(aes(x = sent_rg_id, y = m, color = measure)) +
    ...
    geom_errorbar(aes(ymax = upper, ymin = lower), width = 0.1) +
    geom_line() +
    geom_text(aes(label = cris, y = pval_y + 60), color = "black", size = 2.5) +
