...
# Bayesian hierachical model fitting -- warning: take quite some time to run
```{r, eval=FALSE}
...
## to check the model fits -- warning: take time to run
```{r, eval=FALSE}
...
# Read in model fitting data and prepare for plotting.
```{r}
...
for(exp in c("Coordination", "Relative Clause", "Adverb")) {
  ...
  for(met in c("go_past_time")){
    for(mes in c("MoTR", "G-Maze", "A-Maze", "SPR")) {
    # for(mes in c("SPR")) {
      for (nsubj in c(100, 120, 140, 160, 180, 200)) {
      # for (nsubj in c(60)) {
        target_df <- stats_df_lm %>%filter(experiment == exp, metric == met, measure == mes)
        if(nrow(target_df) > 0) {
          beta_0 <- target_df$beta_0
...
all_power <- read.csv("../data/power_df_simulation.csv") %>%
