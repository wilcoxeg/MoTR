...
clean_df = sample_df %>%
  ...
  mutate(fix_type =  if_else(velocities > 0 & acceleration < 0, "deceleration", fix_type)) %>%
  ...
  mutate(fix_type = factor(fix_type, levels = c("fixation", "deceleration", "movement", "off"), 
                           labels = c("Fixation", "Deceleration", "Other \n Movement", "Offscreen")))
...
movement_df = clean_df %>% filter(subj_id == target_subj_id, item_id == target_item_id) %>%
  mutate(velocities = if_else(velocities > 1, 1, velocities)) %>%
...
ggsave(paste0("../visualizations/reading/reader_",target_subj_id,"_",target_item_id,".pdf"), device="pdf", width = 10, height = 3.2)
