...
fit_gam_inner = function(bootstrap_sample, mean_predictors, target_prev_word, metric, is_bootstrap) {
  ...
  m <- brm(
    ...
    paste0("./bayesian_models/bayesian_models_surprisal/drop0s/feb6_gamma_provo_", metric, "_", bootstrap_sample$id)
  } else {
    paste0("./bayesian_models/bayesian_models_surprisal/drop0s/feb6_gamma_provo_", metric, "_linear_0")
  },
    backend = "cmdstanr",
...
write.csv(plotting_df, "gamma_provo_surprisal_bayesian_2024.csv", row.names = FALSE)
...
ggsave("../visualizations/surp_rt_wt.pdf", device = "pdf", width = 6, height = 2.5)
...
Using linear term for surp and prev_surp would have higher predictive accuracy, e.g. 1.7, 1.4, 1.4, respectively, but the difference is smaller than 4 (suggesting a very small difference) and is smaller than two SE for all the models. So, there is no significant difference between linear and non-linear model.
